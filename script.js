...
// 2. Fungsi Sinkronisasi Waktu (Dynamic Clock)
function updateDateTime() {
    ...
    if(document.getElementById('tgl-update-text')) {
        document.getElementById('tgl-update-text').innerText = `DATA UPDATE: ${days[now.getDay()]}, ${now.getDate()} ${months[now.getMonth()]} ${now.getFullYear()} - ${timeString}`;
    }
...
function processData(data) {
    ...
    // --- AGING BUCKETS (PERBAIKAN LOGIKA DISTRIBUSI) ---
    const buckets = { 'LANCAR': 0, '1-30 H': 0, '31-60 H': 0, '>60 H': 0 };
    data.forEach(d => {
        const aging = String(d.status_aging || '').toUpperCase().trim();
        const val = (Number(d.os_balance) || 0) / 1000000; // Satuan Juta

        if (aging.includes("LANCAR")) {
            buckets['LANCAR'] += val;
        } else if (/1\s*-\s*30/.test(aging)) {
            buckets['1-30 H'] += val;
        } else if (/31\s*-\s*60/.test(aging)) {
            buckets['31-60 H'] += val;
        } else {
            buckets['>60 H'] += val;
        }
    ...
    // --- TOP LEASING BREAKDOWN (PROGRESS BARS) ---
    const leasingMap = {};
    ...
    // --- RENDER LAIN-LAIN ---
    renderListSalesman(data);
    renderListOverdue(data);
    renderCharts(cashNominal, leasingNominal, Object.values(buckets));
}

// 5. Fungsi Render List (Salesman & Overdue)
function renderListSalesman(data) {
    const formatJuta = (n) => (Number(n)/1000000).toFixed(1) + ' Jt';
    const salesMap = {};
    data.forEach(d => {
    ...
    document.getElementById('list-salesman').innerHTML = topSales.map((s, i) => `
        <div class="flex justify-between items-center border-b border-slate-50 pb-2 mb-2">
            ...
            <span class="text-[11px] font-black text-blue-600">${formatJuta(s[1].nominal)}</span>
        </div>`).join('');
}

function renderListOverdue(data) {
    const formatJuta = (n) => (Number(n)/1000000).toFixed(1) + ' Jt';
...
// 6. Visualisasi Grafik (ApexCharts)
function renderCharts(cashVal, leasingVal, agingValues) {
    ...
    donutChart = new ApexCharts(document.querySelector("#chart-donut-main"), {
        ...
        labels: ['Cash', 'Leasing'],
        chart: { type: 'donut', height: 230, fontFamily: 'Public Sans' },
    ...
    // Bar Chart (Aging Analysis) - SESUAI REFERENSI WARNA & POSISI
