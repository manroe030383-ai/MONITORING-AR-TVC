...
async function fetchData() {
    try {
        // Mengambil data dari tabel 'ar_unit' sesuai struktur Supabase Anda
...
function updateDashboard(data) {
    ...
    const now = new Date();
    document.getElementById('status-update').innerText = `DATA UPDATE: ${now.toLocaleString('id-ID')} WIB`;

    // Render Grafik dan List
...
// Fungsi Render untuk Komponen Dashboard
...
function renderLeasingList(map, total) {
    document.getElementById('leasing-list').innerHTML = Object.entries(map).sort((a,b) => b[1] - a[1]).slice(0, 4).map(([n, v]) => `
        <div class="space-y-1">
            <div class="flex justify-between text-[9px] font-bold"><span class="text-slate-500">${n}</span><span class="text-slate-700">${total > 0 ? ((v/total)*100).toFixed(1) : 0}%</span></div>
...
// Menjalankan fetching data saat halaman dimuat
