...
async function fetchData() {
    try {
        ...
        statusEl.innerText = "SINKRONISASI SUPABASE...";
...
function updateDashboard(data) {
    ...
    let mapSales = {}, mapOverdue = {}, mapSpv = {}, mapLeasing = {}, mapSpeed = {};
    ...
    data.forEach(d => {
        ...
        if (["CASH", ""].includes(lName)) {
            ...
            // Simulasi Speed Payment (Biasanya berdasarkan Leadtime tgl Tagih ke tgl Bayar)
    ...
    renderTopList('list-spv', mapSpv, 'text-blue-600');
...
function renderCharts(cash, leas, aging) {
    ...
    if (!charts.donut) {
        charts.donut = new ApexCharts(document.querySelector("#chart-donut-leasing"), {
            ...
            stroke: { width: 0 },
            plotOptions: { pie: { donut: { size: '75%', labels: { show: true, total: { show: true, label: 'TOTAL O/S', fontSize: '9px', fontWeight: 800 } } } } },
            dataLabels: { enabled: false },
...
function renderLeasingList(map, total) {
    document.getElementById('leasing-list').innerHTML = Object.entries(map)
        .sort((a,b) => b[1] - a[1]).slice(0, 4).map(([name, val]) => {
...
function renderSpeedPayment(map) {
    document.getElementById('speed-payment-list').innerHTML = Object.entries(map).slice(0, 3).map(([name, days]) => `
        <div class="space-y-1">
            ...
            <div class="w-full bg-slate-50 h-3 rounded-sm overflow-hidden border border-slate-100">
                <div class="bg-indigo-500 h-full" style="width: ${(days/40)*100}%"></div>
            </div>
        </div>
    `).join('');
}
