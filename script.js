...
async function fetchData() {
    try {
        ...
        if (data && data.length > 0) {
            ...
            console.warn("Data tidak ditemukan atau tabel kosong.");
        }
    } catch (e) {
...
// 1. Update Komponen List (TVC, Top Sales, Overdue, SPV)
function updateExtraComponents(data) {
    ...
    if (elSales) elSales.innerHTML = topSales.map(d => `<div class="text-[10px] mb-2"><div class="font-bold">${d.Customer_Name}</div><div class="text-slate-400">Rp ${cleanNum(d.os_balance).toLocaleString('id-ID')}</div></div>`).join('');

    // Breakdown Leasing TVC
    const tvc = data.filter(d => ['TAFS', 'ACC'].includes(d.Leasing_Name));
    const elTotalTvc = document.getElementById('total-unit-tvc');
    if (elTotalTvc) elTotalTvc.innerText = tvc.length + ' Unit';
    const elGiTvc = document.getElementById('unit-gi-tvc');
    if (elGiTvc) elGiTvc.innerText = tvc.filter(d => d.Status_Aging === 'GI').length + ' Unit';
    const elDelTvc = document.getElementById('unit-delivery-tvc');
    if (elDelTvc) elDelTvc.innerText = tvc.filter(d => d.Status_Aging !== 'GI').length + ' Unit';
