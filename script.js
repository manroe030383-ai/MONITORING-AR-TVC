...
function processData(data) {
    ...
    if (bodyAr) {
        bodyAr.innerHTML = data.map((d, i) => `
            <tr class="border-b">
                <td class="p-4">${i + 1}</td>
                <td class="p-4 font-bold text-sm">${d.Customer_Name || '-'}</td>
                <td class="p-4 text-sm">${d.Leasing_Name || '-'}</td>
                <td class="p-4 text-right">${fmtIDR(d.os_balance)}</td>
                <td class="p-4"><input class="border rounded p-1 w-full" value="${d.ket_cabang || ''}"></td>
                <td class="p-4"><input class="border rounded p-1 w-full" value="${d.plan_bayar_leasing || ''}"></td>
                <td class="p-4"><input class="border rounded p-1 w-full" value="${d.keterangan_leasing || ''}"></td>
                <td class="p-4"><button class="bg-blue-600 text-white px-3 py-1 rounded">Save</button></td>
            </tr>
        `).join('');
    }
    ...
    if (elTotal) {
        elTotal.innerText = fmtIDR(data.reduce((sum, d) => sum + Number(d.os_balance || 0), 0));
    }
}
