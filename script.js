...
function updateDateTime() {
    ...
    const arsipElem = document.getElementById('arsip-db-text');
    
    if(tglElem) tglElem.innerText = `DATA UPDATE: ${days[now.getDay()]}, ${now.getDate()} ${months[now.getMonth()]} ${now.getFullYear()} - ${timeDisplay}`;
    if(arsipElem) arsipElem.innerText = `${now.getDate()}/${now.getMonth() + 1}/${now.getFullYear()}`;
}
...
// 4. Pengolahan Data & Distribusi Aging
function processData(data) {
    ...
    data.forEach(d => {
        ...
        // Klasifikasi Aging (Gunakan RegEx agar tidak meleset)
        const agingTxt = String(d.status_aging || '').toUpperCase();
        if (agingTxt.includes("LANCAR")) {
            buckets['LANCAR'] += val / 1000000;
        } else if (/1\s*-\s*30/.test(agingTxt)) {
            buckets['1-30 H'] += val / 1000000;
        } else if (/31\s*-\s*60/.test(agingTxt)) {
            buckets['31-60 H'] += val / 1000000;
        } else {
            buckets['>60 H'] += val / 1000000;
        }
    });
...
function renderCharts(cash, leasing, agingData) {
    ...
    // Bar Chart Aging (SESUAI REFERENSI)
    barChart = new ApexCharts(document.querySelector("#chart-aging-nominal"), {
        series: [{ name: 'Nominal O/S', data: agingData }],
        chart: { type: 'bar', height: 350, toolbar: { show: false } },
        colors: ['#10B981', '#FFD700', '#FF8C00', '#EF4444'], // Sesuai urutan bucket
        plotOptions: { 
            bar: { distributed: true, borderRadius: 8, columnWidth: '60%', dataLabels: { position: 'top' } } 
        },
        dataLabels: {
            enabled: true,
            offsetY: -20,
            formatter: (v) => v > 0 ? v.toFixed(1) + ' Jt' : '',
            style: { colors: ['#1B2559'], fontWeight: 'bold' }
        },
    ...
    document.getElementById('list-overdue').innerHTML = overdue.map((d, i) => `
        <div class="flex justify-between items-center mb-3">
            <div class="flex flex-col">
                <span class="text-[9px] font-bold text-[#1B2559]">${i+1}. ${d.customer_name}</span>
                <span class="text-[7px] text-red-500 font-bold">MAX ${d.hari_overdue} HARI</span>
            </div>
